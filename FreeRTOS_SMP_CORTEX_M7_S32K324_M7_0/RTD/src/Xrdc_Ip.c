...
#define RM_XRDC_IP_SW_MAJOR_VERSION_C                 1
...
#define RM_START_SEC_CONST_BOOLEAN
#include "Rm_MemMap.h"
#if (XRDC_IP_DEV_ERROR_DETECT == STD_ON)
static const boolean g_bAvailablePidRegisters[XRDC_PID_COUNT] = XRDC_AVAILABLE_PIDS;
#endif
static const uint32 Xrdc_Ip_InstanceAddress[XRDC_COUNT] = XRDC_BASE_ADDRS;

#define RM_STOP_SEC_CONST_BOOLEAN
#include "Rm_MemMap.h"
...
static void Xrdc_Memory_Config_Descriptor(Xrdc_Ip_MemConfigType const * pXrdcMemDescriptor);

static void Xrdc_Peripheral_Access_Config(Xrdc_Ip_PeripheralConfigType const * pXrdcPerDescriptor);

static void Xrdc_Domain_Init(Xrdc_Ip_DomainConfigType const * pDomainConfig);
...
void Xrdc_Ip_SetProcessID_Privileged(uint8 u8MasterCore, uint8 u8Pid, Xrdc_Ip_SecureAttributeType eSecureAttr);

void Xrdc_Ip_LockProcessID_Privileged(uint8 u8MasterCore);
/*==================================================================================================
*                                       LOCAL FUNCTIONS
==================================================================================================*/
...
static void Xrdc_Memory_Config_Descriptor(Xrdc_Ip_MemConfigType const * pXrdcMemDescriptor )
{
    ...
    DevAssert(pXrdcMemDescriptor->u32XrdcInstance < XRDC_COUNT);
#endif
    /* Clear Valid bit of descriptor before starting writing new descriptor */
    MCAL_DATA_SYNC_BARRIER();
    MCAL_INSTRUCTION_SYNC_BARRIER();

    XRDC_REG_RMW32(XRDC_MRGD_W_ADDR32((uint32)pXrdcMemDescriptor->u32XrdcInstance, XRDC_WORD3, \
        (((uint32)pXrdcMemDescriptor->u32XrdcMrcInstance << 4UL) + (uint32)pXrdcMemDescriptor->u32XrdcMrcRegionDescriptor)), XRDC_MDA_W1_VALID_MASK,XRDC_MDA_W1_DESCRIPTOR_DISABLE);

    MCAL_DATA_SYNC_BARRIER();
    MCAL_INSTRUCTION_SYNC_BARRIER();
    /* Define start address bit35:5 according to RM */
    XRDC_REG_WRITE32(XRDC_MRGD_W_ADDR32(pXrdcMemDescriptor->u32XrdcInstance, XRDC_WORD0, \
    ...
    XRDC_REG_WRITE32(XRDC_MRGD_W_ADDR32(pXrdcMemDescriptor->u32XrdcInstance, XRDC_WORD1, \
    ...
    XRDC_REG_WRITE32(XRDC_MRGD_W_ADDR32(pXrdcMemDescriptor->u32XrdcInstance, XRDC_WORD2, \
    ...
    XRDC_REG_RMW32(XRDC_MRGD_W_ADDR32(pXrdcMemDescriptor->u32XrdcInstance, XRDC_WORD3, \
        ((pXrdcMemDescriptor->u32XrdcMrcInstance << 4UL) + pXrdcMemDescriptor->u32XrdcMrcRegionDescriptor)), XRDC_MDA_W1_VALID_MASK | XRDC_MDA_W1_DACP_MASK, XRDC_MDA_W1_DESCRIPTOR_ENABLE | pXrdcMemDescriptor->u32XrdcMemPolicy1);
...
static void Xrdc_Peripheral_Access_Config(Xrdc_Ip_PeripheralConfigType const * pXrdcPerDescriptor)
{
    ...
    DevAssert(pXrdcPerDescriptor->u32XrdcInstance < XRDC_COUNT);
#endif
    /* Clear Valid bit of descriptor before starting writing new descriptor */
    MCAL_DATA_SYNC_BARRIER();
    MCAL_INSTRUCTION_SYNC_BARRIER();

    XRDC_REG_RMW32(XRDC_PDAC_W1_ADDR32(pXrdcPerDescriptor->u32XrdcInstance, pXrdcPerDescriptor->u32XrdcPdacInstance), XRDC_PDAC_W1_VALID_MASK, XRDC_PDAC_W1_DESCRIPTOR_DISABLE);

    MCAL_DATA_SYNC_BARRIER();
    MCAL_INSTRUCTION_SYNC_BARRIER();

    XRDC_REG_WRITE32(XRDC_PDAC_W0_ADDR32(pXrdcPerDescriptor->u32XrdcInstance, pXrdcPerDescriptor->u32XrdcPdacInstance), pXrdcPerDescriptor->u32XrdcSema4Enable | ((pXrdcPerDescriptor->u32XrdcSema4Number<<XRDC_SNUM_SHIFT_U32) & XRDC_SNUM_MASK) | pXrdcPerDescriptor->u32XrdcPerPolicy);

    MCAL_DATA_SYNC_BARRIER();
    MCAL_INSTRUCTION_SYNC_BARRIER();

    XRDC_REG_RMW32(XRDC_PDAC_W1_ADDR32(pXrdcPerDescriptor->u32XrdcInstance, pXrdcPerDescriptor->u32XrdcPdacInstance), XRDC_PDAC_W1_VALID_MASK | XRDC_PDAC_W1_DACP_MASK, XRDC_PDAC_W1_DESCRIPTOR_ENABLE | pXrdcPerDescriptor->u32XrdcPerPolicy1);
...
static void Xrdc_Domain_Init(Xrdc_Ip_DomainConfigType const * pDomainConfig)
{
    ...
    DevAssert(pDomainConfig->u32XrdcInstance < XRDC_COUNT);
#endif
    if (pDomainConfig->XrdcCoreMdacInstance == XRDC_CORE_MASTER)
    {
        ...
        XRDC_REG_WRITE32(XRDC_MDA_W_DFMT0_ADDR32((uint32)pDomainConfig->u32XrdcInstance, (uint32)pDomainConfig->u32XrdcWordDescriptor, (uint32)pDomainConfig->XrdcMdacInstance),(uint32)((uint32)(pDomainConfig->u32XrdcDomainID) & XRDC_MDA_DOMAIN_ID_MASK)| \
        (uint32)(XRDC_DID_FIELD_USED)| \
        ...
        (uint32)XRDC_MDA_W1_DESCRIPTOR_ENABLE \
        );
    ...
    else if (pDomainConfig->XrdcCoreMdacInstance == XRDC_NONCORE_MASTER)
    {
        ...
        XRDC_REG_WRITE32(XRDC_MDA_W_DFMT1_ADDR32(pDomainConfig->u32XrdcInstance, (uint32)pDomainConfig->XrdcMdacInstance), (uint32)((uint32)(pDomainConfig->u32XrdcDomainID) & XRDC_MDA_DOMAIN_ID_MASK)| (XRDC_DID_BYPASS_NOT_USED)| XRDC_MASTER_NONCORE_FORMAT | XRDC_MDA_W1_DESCRIPTOR_ENABLE );
        ...
        XRDC_REG_RMW32(XRDC_MDA_W_DFMT1_ADDR32(pDomainConfig->u32XrdcInstance, (uint32)pDomainConfig->XrdcMdacInstance), (XRDC_MDA_PA_ATTR_MASK|XRDC_MDA_SA_ATTR_MASK), (u32UserAtt|u32SecureAtt));
 ...
 *
 */
/* @implements    Xrdc_Ip_GetDomainID_Privileged_Activity */
uint32 Xrdc_Ip_GetDomainID_Privileged(uint32 u32Instance)
{
    uint32 u32Value;
...
*
*/
/* @implements    Xrdc_Ip_Init_Privileged_Activity */
void Xrdc_Ip_Init_Privileged(Xrdc_Ip_ConfigType const * pXrdcConfig)
{
    ...
    DevAssert(pXrdcConfig->u32DomainConfigCnt > 0UL);
    DevAssert(pXrdcConfig->pMemoryDesConfig != NULL_PTR);
    DevAssert(pXrdcConfig->pPeripheralDescriptorConfig != NULL_PTR);
    DevAssert(pXrdcConfig->pDomainConfig != NULL_PTR);
#endif
    ...
    {
       Xrdc_Memory_Config_Descriptor(&pXrdcConfig->pMemoryDesConfig[u32counter]);
    }

    /* Configure all peripheral descriptors */
    for (u32counter = 0U; u32counter < pXrdcConfig->u32PeripheralConfigCnt; u32counter++)
    {
       Xrdc_Peripheral_Access_Config(&pXrdcConfig->pPeripheralDescriptorConfig[u32counter]);
    }

    /* Init all domains*/
    for (u32counter = 0U; u32counter < pXrdcConfig->u32DomainConfigCnt; u32counter++)
    {
        Xrdc_Domain_Init(&pXrdcConfig->pDomainConfig[u32counter]);
    }
...
*
*/
/* @implements    Xrdc_Ip_SetProcessID_Privileged_Activity */
void Xrdc_Ip_SetProcessID_Privileged(uint8 u8MasterCore, uint8 u8Pid, Xrdc_Ip_SecureAttributeType eSecureAttr)
{
#if (XRDC_IP_DEV_ERROR_DETECT == STD_ON)
    /* Check the input parameters */
    DevAssert(u8MasterCore < XRDC_PID_COUNT);
    DevAssert(u8Pid < XRDC_PID_MAX_VALUE_U8);
    DevAssert(g_bAvailablePidRegisters[u8MasterCore] == TRUE);
#endif
    if (XRDC_REG_BIT_GET32(XRDC_HWCFG2_ADDR32(0),(1UL<<(uint32)u8MasterCore)) == 0UL)
    {
        if (eSecureAttr == XRDC_NON_SECURE)
        {
            ...
            XRDC_REG_RMW32(XRDC_PID_ADDR32((uint32)u8MasterCore), XRDC_PID_FIELD_MASK, ((((uint32)u8Pid) & XRDC_PID_NONSECURE_FIELD_MASK) | ((uint32)0x20)));
        ...
        else
        {
            ...
            MCAL_INSTRUCTION_SYNC_BARRIER();
            SchM_Enter_Rm_RM_EXCLUSIVE_AREA_00();
            XRDC_REG_RMW32(XRDC_PID_ADDR32((uint32)u8MasterCore), XRDC_PID_FIELD_MASK, ((((uint32)u8Pid) & XRDC_PID_NONSECURE_FIELD_MASK)| ((uint32)0x00)));
            SchM_Exit_Rm_RM_EXCLUSIVE_AREA_00();
            MCAL_DATA_SYNC_BARRIER();
            MCAL_INSTRUCTION_SYNC_BARRIER();
        }
...
/**
* @brief         Prohibits writes to PID registers
* @details
*
* @param[in]     u8MasterCore:      Index of configured core master
*
...
*
*/
/*   Xrdc_Ip_LockProcessID_Privileged_Activity */
void Xrdc_Ip_LockProcessID_Privileged(uint8 u8MasterCore)
{
#if (XRDC_IP_DEV_ERROR_DETECT == STD_ON)
    /* Check the input parameters */
    ...
    DevAssert(g_bAvailablePidRegisters[u8MasterCore] == TRUE);
        ...
        XRDC_REG_RMW32(XRDC_PID_ADDR32((uint32)u8MasterCore), XRDC_PID_LK2_MASK, (uint32)0x3 << XRDC_PID_LK2_SHIFT_U32);
...
static void Xrdc_Ip_GetDetailError(uint32 u32Instance, uint8 u8SubInstance, Xrdc_Ip_DomainIDErrStatusType * pXrdcDomainIDErrorStatus)
{
    ...
    if (u32ErrorState >= 2U)
    {
        ...
        {
            pXrdcDomainIDErrorStatus->DomainIDAccessError = XRDC_DOMAIN_UNKNOWN;
        }
...
*
*/
/* @implements    Xrdc_Ip_GetDomainIDErrorStatus_Privileged_Activity */
void Xrdc_Ip_GetDomainIDErrorStatus_Privileged(uint32 u32Instance, Xrdc_Ip_DomainIDErrStatusType * pXrdcDomainIDErrorStatus)
{
    uint8  u8counter;
...
{
#ifdef XRDC_IP_ENABLE_USER_MODE_SUPPORT
    return OsIf_Trusted_Call_Return1param(Xrdc_Ip_GetDomainID_Privileged, u32Instance);
#else
    return Xrdc_Ip_GetDomainID_Privileged(u32Instance);
#endif
}
/**
...
*
*/
void Xrdc_Ip_SetProcessID(uint8 u8MasterCore, uint8 u8Pid, Xrdc_Ip_SecureAttributeType eSecureAttr)
{
#ifdef XRDC_IP_ENABLE_USER_MODE_SUPPORT
    OsIf_Trusted_Call3params(Xrdc_Ip_SetProcessID_Privileged, u8MasterCore, u8Pid, eSecureAttr);
#else
    Xrdc_Ip_SetProcessID_Privileged(u8MasterCore, u8Pid, eSecureAttr);
#endif
}
/**
* @brief        Xrdc_Ip_LockProcessID
* @details
*
* @param[in]     u8MasterCore
*
...
*
*/
void Xrdc_Ip_LockProcessID(uint8 u8MasterCore)
{
#ifdef XRDC_IP_ENABLE_USER_MODE_SUPPORT
    OsIf_Trusted_Call1param(Xrdc_Ip_LockProcessID_Privileged, u8MasterCore);
#else
